...
func (c *Copy) Args() []Arg {
	out := []Arg{
		CommandName("cp"),
	}

	if c.Recursive {
		out = append(out, SingleSwitchFlag("r"))
	}

	if c.Verbose {
		out = append(out, SingleSwitchFlag("v"))
	}

	return append(out, []Arg{PositionalArg(c.Src), PositionalArg(c.Dest)}...)
}
...
func (c *Chmod) Args() []Arg {
	...
	return append(out, []Arg{PositionalArg(c.Mode), PositionalArg(c.Path)}...)
}
